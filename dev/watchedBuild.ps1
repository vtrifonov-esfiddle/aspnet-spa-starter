function WatchedRun($hostPort = 45000) {
    ...
    $ContainerName = "aspnet-react-container.dev"
    docker run --rm -it -p "$($hostPort):5000" -v "$($SourcesPath):/app" --name $ContainerName vtrifonovdocker/aspnet-react-container:dev dotnet watch run --urls "http://0.0.0.0:5000"
}
